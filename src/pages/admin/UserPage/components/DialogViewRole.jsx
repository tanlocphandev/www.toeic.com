...
import { useMutationRemoveRoleGrant } from "@/hooks/role/role.mutation.hook";
import { toast } from "sonner";
import { toastConfigSuccess } from "@/configs/toast.config";

const DialogViewRole = ({
    open = false,
    onClose = () => {},
    selectedRole = null,
    isLoading = false,
    ...
    const mutationRemoveGrant = useMutationRemoveRoleGrant();

    console.log("====================================");
    console.log(`data`, data);
    console.log("====================================");

    const handleRemoveGrant = (row) => {
    ...
    const handleConfirmRemoveRole = () => {
        ...
        mutationRemoveGrant.mutate(payload, {
            onSuccess: () => {
                toast.success("Xóa quyền thành công", toastConfigSuccess);
                revalidate();
            ...
            render: (row) => {
                return row?.grant_action === "create:own" ? (
                    <TooltipBase title={"Xóa quyền (hành động này)"}>
                        <Checkbox checked onCheckedChange={() => handleRemoveGrant(row)} />
                    </TooltipBase>
                ) : null;
            },
    ...
    return (
        <>
            ...
            {selectedRole?.role_name === user?.user_role ? (
                <DialogConfirm
                    open={!!selectedRemoveGrant}
                    onClose={handleCloseDialogConfirm}
                    onConfirm={handleCloseDialogConfirm}
                    title={"Cảnh báo (hành động đang tham chiếu)!"}
                    message={`Bạn không thể xóa vai trò của chính ${selectedRole?.role_name} !`}
                    btnTextConfirm="Đóng"
                />
            ) : (
                <DialogConfirm
                    ...
                    title={"Xóa quyền (hành động đang tham chiếu)"}
                    message={`Bạn có chắc chắn muốn xóa hành động \`${selectedRemoveGrant?.grant_action}\` của tài nguyên \`${selectedRemoveGrant?.resource}\`. Hành động xóa này sẽ làm vai trò \`${selectedRole?.role_name}\` không có quyền này. Hãy cân nhắc cẩn thận trước khi thêm ?`}
                    isPending={mutationRemoveGrant.isPending}
                />
            )}
            ...
            <Dialog open={open} onOpenChange={onClose}>
                <DialogContent className="max-w-5xl h-auto">
                    <DialogHeader>
                        <DialogTitle>Vai trò: {selectedRole?.role_name}</DialogTitle>
